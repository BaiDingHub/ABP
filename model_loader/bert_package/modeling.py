...
class BertEmbeddings(nn.Module):
    ...
    def input_to_words_embeddings(self, input_ids):
        # add by myself
        words_embeddings = self.word_embeddings(input_ids)
        return words_embeddings
    
    def words_embeddings_to_embeddings(self, input_ids, words_embeddings, token_type_ids=None):
        # add by myself
        seq_length = input_ids.size(1)
        position_ids = torch.arange(seq_length, dtype=torch.long, device=input_ids.device)
        position_ids = position_ids.unsqueeze(0).expand_as(input_ids)
    ...
    def get_embeddings(self):
        # add by myself
        return self.word_embeddings.weight.data
...
class BertModel(BertPreTrainedModel):
    ...
    def input_to_embs(self, input_ids):
        # add by myself
        words_embeddings = self.embeddings.input_to_words_embeddings(input_ids)
        return words_embeddings

    def embs_to_output(self, input_ids, words_embeddings, token_type_ids=None, attention_mask=None, output_all_encoded_layers=True):
        # add by myself
        if attention_mask is None:
            attention_mask = torch.ones_like(input_ids)
        if token_type_ids is None:
        ...
        extended_attention_mask = (1.0 - extended_attention_mask) * -10000.0
        embedding_output = self.embeddings.words_embeddings_to_embeddings(input_ids, words_embeddings, token_type_ids=token_type_ids)
        encoded_layers = self.encoder(embedding_output,
                                      extended_attention_mask,
                                      output_all_encoded_layers=output_all_encoded_layers)
        sequence_output = encoded_layers[-1]
...
class BertForSequenceClassification(BertPreTrainedModel):
    ...
    def input_to_embs(self, input_ids, attention_mask=None, token_type_ids=None):
        # add by myself
        embs = self.bert.input_to_embs(input_ids)
        return embs
    
    def embs_to_logit(self, input_ids, embs, attention_mask=None, token_type_ids=None):
        # add by myself
        _, pooled_output = self.bert.embs_to_output(input_ids, embs, token_type_ids=token_type_ids, attention_mask=attention_mask, output_all_encoded_layers=False)
        pooled_output = self.dropout(pooled_output)
        logits = self.classifier(pooled_output)
        return logits
    ...
    def get_embeddings(self):
        # add by myself
        embeddings = self.bert.embeddings.get_embeddings()
        return embeddings
